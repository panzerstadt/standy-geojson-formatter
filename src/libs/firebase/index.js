...
export const BuildGeoJSON = ({
  ...
  useEffect(() => {
    ...
    const getUserData = async user =>
      await getSingleUserDataByDate(user, "2019-08-27");
    ...
    const formatCoordinates = data =>
      data.map(d => [d.location.lng, d.location.lat]);

    const getTrainLine = data => data[0].trainLine;

    const getDepartureTime = data => data[0].departureTime;
        ...
        features: [
          {
            ...
            id: "relation/541074",
            properties: {
              user: "tliqun@gmail.com",
              timeRange: getUserTime(userData),
              trainLine: getTrainLine(userData),
              originalDepartureTime: getDepartureTime(userData)
            },
            geometry: {
              type: "MultiPoint",
              coordinates: formatCoordinates(userData)
            }
          }
        ]
      };
...
export const FireStoreDEBUG = ({
  ...
  useEffect(() => {
    ...
    const convertRawDataToMatrix = data => {
      ...
      // if there's literally nothing after cleaned, return early
      if (cleaned.length === 1) {
        const { x, y } = cleaned[0];
        if (x === -1 && y === -1)
          return { error: "all coordinates are out of bounds." };
      }
